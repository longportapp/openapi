...
- **Rust:** new public `longport::counter` module — `symbol_to_counter_id`, `index_symbol_to_counter_id`, `counter_id_to_symbol`, and `is_etf`, backed by the embedded ETF + index + warrant directory, so downstream consumers (CLI / MCP) no longer need their own copies
- **Rust:** `QuoteContext` gains `symbol_to_counter_ids(symbols)` (batch conversion via `POST /v1/quote/symbol-to-counter-ids`) and `resolve_counter_ids(symbols)` (local-first resolution with remote fallback) — remotely resolved entries are persisted to `~/.longport/cache/counter-ids.csv` (one counter_id per line, override the directory with `LONGPORT_CACHE_DIR`) and consulted by subsequent `counter` lookups, so symbols missing from the embedded directory (e.g. newly listed ETFs) resolve correctly after the first query
...
- 19 new APIs: `FundamentalContext` +9, `QuoteContext` +1 (`short_trades`), `MarketContext` +3, new `ScreenerContext` +5 — see PR [#526](https://github.com/longportapp/openapi/pull/526), [#527](https://github.com/longportapp/openapi/pull/527)
...
- **All languages (Rust, Python, Node.js, Java, C, C++):** Restore `Config::refresh_access_token` (and `refresh_access_token_blocking` in Rust). Refreshes the access token via the Longport token-refresh API. Only available with **Legacy API Key** authentication (`Config::from_apikey`); not supported in OAuth 2.0 mode.
...
- **Rust:** Fix copy-paste field mapping bugs in `TryFrom<quote::FilterWarrant> for WarrantInfo` where `strike_price`, `itm_otm`, `implied_volatility`, `delta`, `effective_leverage`, `conversion_ratio`, and `balance_point` were incorrectly mapped to `last_done`. ([#485](https://github.com/longportapp/openapi/pull/485))

# [4.0.3]

## Changed

- OAuth base URL: `openapi.longportapp.com`.
- CN endpoint URLs: `longport.cn`.
- OAuth token storage path: `~/.longport/openapi/`.
- README docs use `openapi.longportapp.com` for OAuth registration endpoints.
- Update proto submodule with latest upstream changes.
...
- **Python:** Fix `str()` on enum fields (e.g. `CashFlow.direction`, `Subscription`, `OptionDirection`) causing a hang/deadlock by registering previously missing types in the quote and trade modules. ([#476](https://github.com/longportapp/openapi/issues/476))

# [4.0.0]

## Added

- **OAuth 2.0** authentication for all language bindings (Rust, C, C++, Java, Python, Node.js). Use `OAuthBuilder` to run the browser flow; pass the resulting `OAuth` handle to `Config::from_oauth()`. Tokens are persisted under `~/.longport/openapi/tokens/<client_id>` and reused; the browser is only opened when no valid token exists.
