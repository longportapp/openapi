...
impl Config {
    ...
    /// ``LONGPORT_LANGUAGE``         from env if omitted; default:
    ...
    fn from_apikey(
        ...
        let mut config = longport::Config::from_apikey(app_key, app_secret, access_token);
    ...
    ///     - ``LONGPORT_LANGUAGE`` - Language identifier, ``zh-CN``, ``zh-HK``
    ///       or ``en`` (Default: ``en``)
    ...
    fn from_oauth(
        ...
        let mut config = longport::Config::from_oauth(oauth.0.clone());
