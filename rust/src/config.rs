...
use longport_httpcli::{HttpClient, HttpClientConfig, Json, Method, is_cn};
...
const DEFAULT_QUOTE_WS_URL_CN: &str = "wss://openapi-quote.longport.cn/v2";
const DEFAULT_TRADE_WS_URL_CN: &str = "wss://openapi-trade.longport.cn/v2";
...
fn env_var(suffix: &str) -> Option<String> {
    std::env::var(format!("LONGPORT_{suffix}")).ok()
}
...
impl Config {
    ...
    /// `LONGPORT_LANGUAGE`, `LONGPORT_QUOTE_WS_URL`,
    /// `LONGPORT_TRADE_WS_URL`, `LONGPORT_ENABLE_OVERNIGHT`,
    /// `LONGPORT_PUSH_CANDLESTICK_MODE`,
    /// `LONGPORT_PRINT_QUOTE_PACKAGES`, `LONGPORT_LOG_PATH`) are read from
    ...
    ///
    ///     let (ctx, receiver) = longport::quote::QuoteContext::new(config);
    ///     Ok(())
    /// }
    /// ```
