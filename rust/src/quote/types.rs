    ...
    fn try_from(info: quote::FilterWarrant) -> Result<Self> {
        ...
        match r#type {
            ...
            WarrantType::Call | WarrantType::Put => Ok(Self {
                ...
                itm_otm: info.itm_otm.parse().ok(),
                implied_volatility: info.implied_volatility.parse().ok(),
                delta: info.delta.parse().ok(),
                call_price: info.call_price.parse().ok(),
                to_call_price: info.to_call_price.parse().ok(),
                effective_leverage: info.effective_leverage.parse().ok(),
